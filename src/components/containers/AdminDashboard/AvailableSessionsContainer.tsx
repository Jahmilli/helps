...
const AvailableSessionsContainer: React.FunctionComponent<
	AvailableSessionsContainerProps
> = props => {
	...
	const renderWaitingList = (session: ISession) => {
		...
		return (
			<Button onClick={addToWaitingList(session)}>
				{session.waitingList.length} Student(s)
			</Button>
		);
	};
			...
			setState({
				columns: [
					...
					{ title: "Type", field: "type" },
					{
						...
						field: "studentId",
						editable: "never",
						render: (rowData: ISession) => <div>{displayStudentId(rowData)}</div>
					},
					{
						title: "Waiting",
						field: "waitingList",
						render: (rowData: ISession) => <div>{renderWaitingList(rowData)}</div>
					}
				],
